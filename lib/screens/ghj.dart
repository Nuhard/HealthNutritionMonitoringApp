...
class _SymptomCheckerScreenState extends State<SymptomCheckerScreen> {
                        ...
                        children: [
                          ...
                          ListTile(
                            ...
                            onTap: () async {
                              ...
                              if (picked != null) {
                                setStateDialog(() => symptom.onsetDate = picked);
                              }
                            },
                          ),
                          if (tempSymptoms.length > 1)
                            Align(
                              ...
                              child: IconButton(
                                ...
                                onPressed: () =>
                                    setStateDialog(() => tempSymptoms.removeAt(index)),
                              ),
                            ),
                        ],
                      ),
  ...
  // Always fetch suggestions for this symptomName
    ...
    child: FutureBuilder<QuerySnapshot>(
      ...
      builder: (context, sugSnap) {
        ...
        final String alertMessage =
            suggestion?['alertMessage'] ?? "This symptom may need medical attention";
        ...
        // Severity-specific icon & tooltip
        IconData? icon;
        Color? iconColor;
        String? tooltipMsg;
...
if (diet.isNotEmpty) ...[
    ...
    child: Align(
      ...
      child: Text(
        ...
        style: TextStyle(
          fontSize: 16,
          fontWeight: FontWeight.bold,
          color: Color.fromARGB(255, 227, 124, 5),
        ),
      ),
    ),
  ),
  ...diet.map(
    (d) => ListTile(
      dense: true,
      contentPadding: const EdgeInsets.symmetric(horizontal: 20, vertical: 2),
      leading: const Icon(Icons.restaurant_outlined, color: Colors.orange),
      title: Text(
        d,
        style: const TextStyle(fontSize: 14),
      ),
  ...
  const Divider(thickness: 1, indent: 16, endIndent: 16),
            ...
            if (suggestion == null && sugSnap.hasData)
              const Padding(
                padding: EdgeInsets.all(12),
                child: Text("No recommendations found for this symptom."),
              ),
          ],
        );
      },
        ...
        child: Column(
          children: [
              ...
              child: StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.hasError) {
                    return Center(
                        child:
                            Text("Error fetching symptoms: ${snapshot.error}"));
                  }
            ...
            ElevatedButton.icon(
              ...
              icon: const Icon(Icons.add),
              label: const Text("Add Symptom(s)"),
